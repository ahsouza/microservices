...
#WORKDIR /var/www
WORKDIR /usr/lib
COPY /.extras/*.zip /usr/lib/

ENV DOCKERIZE_VERSION v0.6.1
RUN wget https://github.com/jwilder/dockerize/releases/download/$DOCKERIZE_VERSION/dockerize-alpine-linux-amd64-$DOCKERIZE_VERSION.tar.gz \
    && tar -C /usr/lib/bin -xzvf dockerize-alpine-linux-amd64-$DOCKERIZE_VERSION.tar.gz \
...
RUN apk add openssl php7-dev php7-pear gcc musl-dev musl unzip nodejs npm libnsl libarchive-tools libaio bash tar unzip g++ && \
	...
	unzip basic.zip && \
	unzip sdk.zip && \
	ln -s /usr/lib/instantclient_10_2 /usr/lib/instantclient && \
	ln -s /usr/lib/instantclient/libclntsh.so.* /usr/lib/instantclient/libclntsh.so && \
	ln -s /usr/lib/instantclient/lib* /usr/lib && \
	docker-php-ext-configure oci8 --with-oci8=instantclient,/usr/lib/instantclient && \
	docker-php-ext-install oci8 && \
	docker-php-ext-enable oci8 && \
	rm -rf /var/lib/apk/lists/*

ENV ORACLE_BASE /usr/lib/instantclient_10_2
ENV LD_LIBRARY_PATH /usr/lib/instantclient_10_2
ENV TNS_ADMIN /usr/lib/instantclient_10_2
ENV ORACLE_HOME /usr/lib/instantclient_10_2
...
#WORKDIR /usr/lib/etc/php/conf.d
#RUN ls -a
